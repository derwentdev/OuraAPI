...
**Why Upstash?** Oura issues a brand-new refresh token every time your
server refreshes its access token, invalidating the old one (this is
standard OAuth token rotation). Your server only kept the new one in
memory — fine, until Render's free tier spins the server down after
inactivity and wipes that memory. Upstash gives the server somewhere
durable to save the current token, so a restart reloads the latest one
instead of falling back to a stale one.
...
- **Token rotation is automatic** with Upstash in place — you should
  never need to manually run `oura_recovery.py` again after initial
  setup.
